...
public class AccumulateTest
{
    ...
    [Test]
    public void Accumulate_mod_2()
    {
        Assert.That(new[] { 1, 4, 6 }.Accumulate(x => x % 2), Is.EqualTo(new[] { 1, 0, 0 }));
    }

    [Test]
    public void Accumulate_upcases_Use_Lambda()
    {
    ...
    [Test]
    public void Accumulate_upcases_no_Lambda()
    {
        Assert.That(new List<string> { "hello", "world" }.Accumulate(AccumulateToUpper),  
            Is.EqualTo(new List<string> { "HELLO", "WORLD" }));
    }
        public static string AccumulateToUpper(string x)
        {
            return x.ToUpper();
        }
